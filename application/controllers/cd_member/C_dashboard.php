...
class C_dashboard extends CI_Controller 
{
   public function __construct() {
    ...
    public function index() 
    {
        ...
        $data['title'] = 'Siap 98 - Kabinet Member (Dashboard)';
        $data['nama_lengkap'] = VERBOSE(XDB(1,'GET','t_users',array('user_id'=>$user_id))['nama']);
        $bonus = XDB(1,'GET','t_users_bonus',array('user_id'=>$user_id));
        $bonus['akumulasi_bonus'] = number_format($bonus['akumulasi_bonus']) ;
        $data['bonus'] = $bonus ;
        $downline = XDB(1,'GET','t_users_downline',array('user_id'=>$user_id));
        $board = $downline['board'];
        $table = 't_board_'.$board ;
         $sql = "SELECT id, nrow FROM (SELECT *, @rownum:=@rownum + 1 AS nrow FROM $table, (SELECT @rownum:=0) r 
        ORDER BY id ) d WHERE d.user_id=? " ;
        $box1 = $this->db->query($sql,$user_id);
        $data['urutan'] = $box1->row_array();
        ...
        if ($downline['total_downline'] < $limit)
          $data['is_manager'] = '<a href=""><font color="red"> Awas! Sebelum sponsoring 100 urutan bisa tersusul!</font></a>';
        else
          $data['is_manager'] = 'Posisi Anda Aman!'; 

        // data topsponsor
        $sql = "SELECT MAX(total_downline) as maxboard FROM t_users_downline WHERE board=$board ";
        $data['maxboard'] = $this->db->query($sql)->row_array()['maxboard'];

        $sql = "SELECT MAX(total_downline) as maxdown FROM t_users_downline ";
        $data['maxdown'] = $this->db->query($sql)->row_array()['maxdown'];
